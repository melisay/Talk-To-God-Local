...
# Removed Vosk-based recognition.
# Instead, we use SpeechRecognition with Google Speech API.

def listen_to_user():
    """Listen for user input using SpeechRecognition with the default microphone."""
    recognizer = sr.Recognizer()
    with sr.Microphone() as source:
        try:
            ...
            debug_log("Speech recognition timeout.")
            return ""
        except sr.UnknownValueError:
